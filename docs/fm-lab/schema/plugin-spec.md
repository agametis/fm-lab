...
Unlike fm-spec, which is built in a sister project and deployed as an artifact, plugin-spec is **derived locally** from the installed [MBS documentation mirror](../docsets/Doc%20Set%20mbs.md): `tools/plugin-spec/derive_mbs.py` parses the per-function platform tables of the mirror and rebuilds `reference/plugin_spec.duckdb` deterministically (the installer runs it automatically after a docs update; two runs over the same mirror produce bit-identical content). The REST API attaches it read-only under the alias `plugref`, next to fm-spec's `ref`.
