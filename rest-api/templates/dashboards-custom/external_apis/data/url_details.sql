...
classified AS (
    SELECT
        ...
        CASE
            ...
            WHEN regexp_matches(url, 'https?://(192\.168|127\.0|10\.|172\.(1[6-9]|2[0-9]|3[01])\.)')  THEN 'Internal LAN'
            WHEN url ILIKE '%localhost%' OR url ILIKE '%0.0.0.0%'                                     THEN 'Localhost'
            -- Fallback for Import Records hits that do not carry an http URL
            -- (file:…, $Variable, fmnet:…, empty source).
            WHEN source_type = 'Script (Import)'                                                      THEN 'Local Import'
            ELSE 'Other'
        END AS api_family,
        CASE
