...
SELECT
    ...
    in_comment
FROM classified
-- Kommentar-Filter (getvariable('comment'), Default 'exclude'): steuert die
-- FilterChips im Kartenkopf. 'all' = alle · 'exclude' = nur aktiver Code (Default,
-- blendet Kommentar-False-Positives aus) · 'only' = nur Kommentar-Treffer.
WHERE (CASE COALESCE(getvariable('comment'), 'exclude')
           WHEN 'all'  THEN TRUE
           WHEN 'only' THEN in_comment
           ELSE NOT in_comment
       END)
-- Filter comparisons accept both the canonical English value ('All') and the
-- legacy German one ('Alle') so existing bookmarked URLs keep resolving to the
-- "filter cleared" state.
  ...
  -- Host-Drilldown: Klick auf eine Zeile der konsolidierten Host-Liste (url_hosts)
