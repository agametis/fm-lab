...
-- and the header slider's dynamic upper bound (max_portal_count = the highest value
...
threshold AS (SELECT CAST(COALESCE(getvariable('min_portals'), '6') AS INTEGER) AS t)
SELECT
    ...
    COALESCE(MAX(cnt), 0)                                                     AS max_portal_count
FROM per_group;
