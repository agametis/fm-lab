...
-- Formula source is the CalculationsCatalog hide instance of the object
-- (single source for all calculation slots); LayoutObjects contributes the
-- geometry and typing of the owning object.
SELECT 'layout-quoted-hide-calc' AS rule_id, 'error' AS severity,
    ...
    'Hide condition is a quoted string constant, not a calculation — ' || trim(COALESCE(c.Formula_Text, c.Display_Text)) AS message,
