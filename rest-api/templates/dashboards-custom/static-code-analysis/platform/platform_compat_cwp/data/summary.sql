-- Hand-maintained COUNT wrapper embedding the findings core of rule (platform_compat_cwp).
...
FROM (
SELECT 'platform-cwp' AS rule_id,
    CASE WHEN c.step_id IS NULL THEN 'info'
         WHEN c.cwp = false THEN 'error'
    ...
    CASE WHEN c.step_id IS NULL
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' has no published compatibility row for Custom Web Publishing (genuine gap in the Claris table)'
         WHEN c.cwp = false
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' does not run on Custom Web Publishing'
         ELSE COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' is PARTIALLY supported on Custom Web Publishing - conditionally supported, see the step notes on its Claris help page'
    END AS message,
...
WHERE s.Is_Enabled
  ...
  AND (c.step_id IS NULL OR c.cwp = false OR c.cwp IS NULL)
