-- Hand-maintained COUNT wrapper embedding the findings core of rule (platform_compat_ios).
...
FROM (
SELECT 'platform-ios' AS rule_id,
    ...
    CASE WHEN c.step_id IS NULL
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' has no published compatibility row for FileMaker Go (iOS) (genuine gap in the Claris table)'
         WHEN c.go = false
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' does not run on FileMaker Go (iOS)'
         ELSE COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' is PARTIALLY supported on FileMaker Go (iOS) - conditionally supported, see the step notes on its Claris help page'
    END AS message,
...
WHERE s.Is_Enabled
  ...
  AND (c.step_id IS NULL OR c.go = false OR c.go IS NULL)
