-- Plugin-function compatibility for Custom Web Publishing (axis a, plugin evidence).
...
core AS (
    SELECT 'platform-plugins-cwp' AS rule_id,
        ...
        CASE WHEN e.canonical_name IS NULL
             ...
             ELSE e.plugin_prefix || ' function ' || e.canonical_name || ' does not run in Custom Web Publishing script execution (runs server-side under the FileMaker Server script engine)'
    ...
    JOIN plugref.plugin_runtime_map rm
      ON rm.plugin_id = e.plugin_id AND rm.fm_env = 'cwp' AND rm.predicate = 'server=false'
