-- Plugin-function compatibility for FileMaker Server (axis a, plugin evidence).
...
-- the MBS docs mirror. Interpretation layer: curated plugin_runtime_map row
-- (predicate server=false -> this environment, provenance per row).
-- Severity model: mapped effect ('error') for functions the map excludes from
-- this environment; unknown function of a known plugin -> 'info' (unresolved,
-- never a silent fall-through); Server=Yes with an OS gap on the server OS
-- set -> 'warning' with finding_kind='os_conditional' (v7 cross-refinement:
-- runs on the server, but not on Linux FMS). MBS platform flags are binary
-- with MBS authority - independent of the Claris tri-state step_compat
-- semantics.
...
enriched AS (
    SELECT n.*, fc.component
    FROM named n
    LEFT JOIN plugref.plugin_functions fc
           ON fc.plugin_id = n.plugin_id AND fc.function_name = n.canonical_name
),
-- Server x OS cross-refinement (v7): a function with Server=Yes whose OS
-- flags do not cover every OS FileMaker Server runs on (fm_spec host matrix
-- ref.runtime_os_matrix; practically linux=false) is only CONDITIONALLY
-- server-compatible - it runs on macOS/Windows FMS but fails on Linux-based
-- FileMaker Server (a standard deployment). The OS tuple folds through the
-- curated plugref.plugin_os_map; 'ios' is never a server OS.
os_condition AS (
...
core AS (
    SELECT 'platform-plugins-server' AS rule_id,
        CASE WHEN e.canonical_name IS NULL THEN 'info'
             WHEN oc.function_name IS NOT NULL THEN 'warning'
             ELSE rm.effect END AS severity,
        CASE WHEN e.canonical_name IS NULL THEN 'unresolved'
             WHEN oc.function_name IS NOT NULL THEN 'os_conditional'
             ELSE 'incompatible' END AS finding_kind,
        ...
        e.component, oc.os_condition, e.via_custom_function, e.evidence_kind, e.usage_count,
        CASE WHEN e.canonical_name IS NULL
             THEN 'Unresolved plugin function "' || e.raw_name || '" (' || e.plugin_prefix || ') - no entry in the plugin platform map (doc mirror outdated or function renamed?); no compatibility statement available'
             WHEN oc.function_name IS NOT NULL
             THEN e.plugin_prefix || ' function ' || e.canonical_name || ' runs under the FileMaker Server script engine, but only on ' || oc.os_condition || ' servers - it fails on FileMaker Server hosts running ' || oc.os_missing
                  || CASE WHEN e.via_custom_function IS NOT NULL
                          THEN ' - called via custom function "' || e.via_custom_function || '"' ELSE '' END
             ELSE e.plugin_prefix || ' function ' || e.canonical_name || ' does not run under the FileMaker Server script engine (schedules, Perform Script on Server)'
                  || CASE WHEN e.via_custom_function IS NOT NULL
                          THEN ' - called via custom function "' || e.via_custom_function || '"' ELSE '' END
        END AS message
    FROM enriched e
    JOIN plugref.plugin_runtime_map rm
      ON rm.plugin_id = e.plugin_id AND rm.fm_env = 'server' AND rm.predicate = 'server=false'
    LEFT JOIN plugref.plugin_function_platforms pf
      ON pf.plugin_id = e.plugin_id AND pf.function_name = e.canonical_name AND pf.platform = 'server'
    LEFT JOIN os_condition oc
      ON oc.plugin_id = e.plugin_id AND oc.function_name = e.canonical_name
     AND pf.supported = true
    WHERE e.canonical_name IS NULL OR pf.supported = false OR oc.function_name IS NOT NULL
