-- Hand-maintained COUNT wrapper embedding the findings core of rule (platform_compat_server).
...
FROM (
SELECT 'platform-server' AS rule_id,
    ...
    CASE WHEN c.step_id IS NULL
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' has no published compatibility row for FileMaker Server (genuine gap in the Claris table)'
         WHEN c.server = false
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' does not run on FileMaker Server'
         ELSE COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' is PARTIALLY supported on FileMaker Server - conditionally supported, see the step notes on its Claris help page'
    END AS message,
