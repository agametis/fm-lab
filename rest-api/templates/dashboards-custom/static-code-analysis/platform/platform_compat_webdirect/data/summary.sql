-- Hand-maintained COUNT wrapper embedding the findings core of rule (platform_compat_webdirect).
...
FROM (
SELECT 'platform-webdirect' AS rule_id,
    ...
    CASE WHEN c.step_id IS NULL
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' has no published compatibility row for FileMaker WebDirect (genuine gap in the Claris table)'
         WHEN c.webdirect = false
         THEN COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' does not run on FileMaker WebDirect'
         ELSE COALESCE(st.canonical_name, 'Step ' || s.Step_ID) || ' is PARTIALLY supported on FileMaker WebDirect - conditionally supported, see the step notes on its Claris help page'
    END AS message,
...
WHERE s.Is_Enabled
  ...
  AND (c.step_id IS NULL OR c.webdirect = false OR c.webdirect IS NULL)
